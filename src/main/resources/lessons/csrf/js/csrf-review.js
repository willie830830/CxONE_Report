    ...
    function getChallenges() {
        ...
        $.get('csrf/review', function (result, status) {
            for (var i = 0; i < result.length; i++) {
                ...
                comment = comment.replace('COMMENT', escapeHtml(result[i].text));   // use escapeHtml function to neutralize potential scripts
                comment = comment.replace('STARS', escapeHtml(result[i].stars));    // use escapeHtml function to neutralize potential scripts
                $("#list").append(comment);
            }
        });
    }
